...
**Goal:** Stand up a second step-ca instance on the existing RADIUS VMs with an RSA-2048 KMS-HSM issuing intermediate (under the existing EC root), serving SCEP to both Windows and non-ADE/DEP Macs, without touching the EC ACME path or the 460 attested-ACME Macs.

**Architecture:** A new Cloud KMS RSA signing key + new Secret Manager secrets + a new GCLB stack (`ca-rsa.campusgroup.co`:8444) are added in Terraform. `startup.sh` gains a second CA bootstrap block (mirroring the EC one) that inits/restores the RSA intermediate + RSA SCEP decrypter, renders a second `ca.json`, and runs a `step-ca-rsa.service` on :8444. FreeRADIUS adds the RSA intermediate to its client-cert trust bundle. Fleet profiles for Windows and non-ADE Macs point at the new RSA endpoint.
